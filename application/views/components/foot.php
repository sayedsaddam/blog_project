		<div class="jumbotron bg-dark text-light mb-0 mt-5 rounded-0">
			<div class="container">
				<div class="row">
					<div class="col-md-4">
						<h5>About</h5>
						<p class="font-weight-light">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Dolorum quo ipsa officiis laudantium porro assumenda accusamus reiciendis, omnis laboriosam consequatur sed sint, hic laborum beatae, expedita esse unde. Omnis, porro!</p>
					</div>
					<div class="col-md-2 font-weight-light">
						<h5>Quick Links</h5>
						<li class="mb-0"><a class="text-light" href="#">Home</a></li>
						<li class="mb-0"><a class="text-light" href="#">About</a></li>
						<li class="mb-0"><a class="text-light" href="#">Contact</a></li>
						<li class="mb-0"><a class="text-light" href="#">Careers</a></li>
						<li class="mb-0"><a class="text-light" href="#">Blog</a></li>
					</div>
					<div class="col-md-2 font-weight-light">
						<h5>Politics</h5>
						<li><a href="" class="text-light">Losers</a></li>
						<li><a href="" class="text-light">Pol. Idiots</a></li>
						<li><a href="" class="text-light">Demo-crazy</a></li>
						<li><a href="" class="text-light">Horse Trading</a></li>
						<li><a href="" class="text-light">Conspiracies</a></li>
					</div>
					<div class="col-md-4">
						<h5>Find Us <small class="font-weight-light">All over the place</small></h5>
						<p class="font-weight-light">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quis deserunt culpa distinctio perspiciatis fugit nam voluptatem fugiat corporis ipsam est ea minus magnam sed hic tenetur ipsa accusantium, quas eligendi.</p>
					</div>
				</div>
			</div>
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12 text-right">
						<small class="">&copy; Copyright <?= date('Y'); ?> WatchZone. All rights reserved. Yes, all of them.</small>
					</div>
				</div>
			</div>
		</div>
		<script src="<?= base_url('assets/js/popper.min.js'); ?>"></script>
	</body>
</html>
